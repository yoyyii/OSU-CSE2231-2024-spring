...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        if (leftIndex <= last) {
            ...
            if (rightIndex <= last) {
                /*
                 * check relationship between left and right
                 */

                /*
                 * if left is greater than or equals to right, compare root with
                 * right
                 */
                if (array[leftIndex] >= array[rightIndex]) {

                    //if root is greater than right, swap them

                    if (array[currIndex] > array[rightIndex]) {
                        exchangeEntries(array, currIndex, rightIndex);
                        siftDown(array, rightIndex, last);
                    }

                    //if not, we're done

                } else { // compare root with left

                    //if root is greater than left, swap them

                    if (array[currIndex] > array[leftIndex]) {
                        exchangeEntries(array, currIndex, leftIndex);
                        siftDown(array, leftIndex, last);
                    }
                    //if not, we're done
                }

            } else { //only compare with left child
