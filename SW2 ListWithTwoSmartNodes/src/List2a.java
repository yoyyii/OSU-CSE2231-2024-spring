...
public class List2a<T> extends ListSecondary<T> {
    ...
    public final void addRightFront(T x) {
        ...
        Node add = new Node();
        add.data = x;
        add.next = this.lastLeft.next;
        this.lastLeft.next = add;
        this.rightLength++;
    ...
    private final class List2aIterator implements Iterator<T> {
        ...
        public boolean hasNext() {
            //check whether we have reached the second smart node or not
    ...
    /*
     * since now we have a second smart node in the back, we can't jump straight
     * to the back anymore. Thus, the only thing we could do is manually
     * advancing the iterator, which is not a faster implementation
     */
