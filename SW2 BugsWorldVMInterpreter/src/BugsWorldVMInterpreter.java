...
public final class BugsWorldVMInterpreter {
    ...
    public static int nextPrimitiveInstructionAddress(int[] cp, CellState wbs,
        ...
        if (!isPrimitiveInstructionByteCode(cp[pc])) {
            /*
             * if current position is unconditional jump, step one step forward
             * to get the address of where to jump to
             */
            if (cp[pc] == Instruction.JUMP.byteCode()) {
                address = nextPrimitiveInstructionAddress(cp, wbs, cp[pc + 1]);

            } else if (conditionalJumpCondition(wbs, cp[pc])) {
                /*
                 * if it's conditional jump and the condition have been met,
                 * step forward to steps into the actual body. then do a
                 * recursive call on it
                 */
                address = nextPrimitiveInstructionAddress(cp, wbs, pc + 2);
            } else {
                /*
                 * if the condition for conditional jump have not been met (i.e.
                 * jump), then step forward by one to get the address for where
                 * to jump to
                 */
                address = nextPrimitiveInstructionAddress(cp, wbs, cp[pc + 1]);
            }
