...
public final class Program1PrettyPrint1 extends Program1 {
    ...
    public void prettyPrint(SimpleWriter out) {
        ...
        Map<String, Statement> notAccessed = this.newContext();
        Map<String, Statement> accessed = this.newContext();

        this.swapContext(notAccessed);

        while (notAccessed.size() > 0) {
            Map.Pair<String, Statement> p = notAccessed.removeAny();
            ...
            accessed.add(p.key(), p.value());
        }
        out.println("BEGIN");
        this.swapContext(accessed);
