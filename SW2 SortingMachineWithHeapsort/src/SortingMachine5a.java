...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        if (leftIndex < last
                && order.compare(array[smallestIndex], array[leftIndex]) > 0) {
            smallestIndex = leftIndex;
        }

        if (rightIndex < last
                && order.compare(array[smallestIndex], array[rightIndex]) > 0) {
            smallestIndex = rightIndex;
        }

        if (smallestIndex != top) {
            exchangeEntries(array, smallestIndex, top);
        }
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        /*
         * heapify left subtree, then right subtree, then swiftdown to make sure
         * root is in right place
         */
        ...
        //make sure right exist
        if (rightIndex < last) {
            heapify(array, rightIndex, order);

        }

        siftDown(array, currIndex, array.length - 1, order);
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        /*
         * add all elements from queue into heap, then heapify it to make sure
         * it's a valid heap
         */
