...
public final class TagCloud {
    ...
    /**
     * Build word comparator for sorting in alphabetical order. All the letter
     * is in lower case.
     */
    private static class WordComparator
        ...
        public int compare(Map.Pair<String, Integer> map1,
            ...
            String map1Str = map1.key();
            String map2Str = map2.key();
            return map1Str.compareTo(map2Str);
        }
    }
    ...
    public static void printFont(SimpleWriter out,
            Map.Pair<String, Integer> pair, int max, int min) {
        ...
        if (max != min) {
            double fontDouble = (48.0 - 11.0) / ((double) max - (double) min)
                    * ((double) count - (double) min);
            int fontNumber = (int) fontDouble + 11;
            String font = "f" + fontNumber;
            out.println("<span style=\"cursor:default\" class=\"" + font
                    + "\" title=\"count: " + count + "\">" + word + "</span>");
        } else {
            out.println(
                    "<span style=\"cursor:default\" class=\"f11\" title=\"count: "
                            + count + "\">" + word + "</span>");
        }
    ...
    /**
     * Print the heading of HTML file.
     *
     * @param out
     *            the output stream
     * @param title
    ...
    public static void printCloud(SimpleWriter out, String folder,
        ...
        final int size = wordSort.size();
        for (int i = 0; i < size; i++) {
            Map.Pair<String, Integer> temp = wordSort.removeFirst();
            printFont(out, temp, maxCount, minCount);
        }
        texts.close();
    }
