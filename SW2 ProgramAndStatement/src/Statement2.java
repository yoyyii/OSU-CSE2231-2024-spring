...
public class Statement2 extends StatementSecondary {
    ...
    public final void addToBlock(int pos, Statement s) {
        ...
        // TODO - fill in body
        Sequence<Tree<StatementLabel>> blockChild = this.rep
                .newSequenceOfTree();
        StatementLabel root = this.rep.disassemble(blockChild);
        Statement2 S = (Statement2) s;
        blockChild.add(pos, S.rep);
        this.rep.assemble(root, blockChild);
        S.createNewRep(); //clear S.
    }
    ...
    public final void assembleIfElse(Condition c, Statement s1, Statement s2) {
        ...
        // TODO - fill in body
        Statement2 S1 = (Statement2) s1;
        Statement2 S2 = (Statement2) s2;
        StatementLabel label = new StatementLabel(Kind.IF_ELSE, c);
        Sequence<Tree<StatementLabel>> blockChild = this.rep
                .newSequenceOfTree();
        blockChild.add(0, S1.rep);
        blockChild.add(1, S2.rep);
        this.rep.assemble(label, blockChild);
        S1.createNewRep();
        S2.createNewRep();
    }
    ...
    public final Condition disassembleIfElse(Statement s1, Statement s2) {
        ...
        Sequence<Tree<StatementLabel>> blockChild = this.rep
                .newSequenceOfTree();
        StatementLabel label = this.rep.disassemble(blockChild);
        S1.rep = blockChild.remove(0);
        S2.rep = blockChild.remove(0);
        this.createNewRep(); // clears this
        return label.condition;
    }
